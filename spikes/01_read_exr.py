...
parser = argparse.ArgumentParser(
    description="Script for get metadata from exr file"
)  # create argument parser
parser.add_argument(
    "filepath", type=Path, help="Path to needed exr file"
...
try:
    ...
    if buffer.has_error:
        error_msg = buffer.geterror()
        print(f"Failed to read image from {filepath}. OIIO error: {error_msg}")
        sys.exit(1)
...
except oiio.ImageBufError as e:
...
compression = spec.get_string_attribute(
    "compression", "unknown"
)  # get compression type
file_size_bytes = os.path.getsize(filepath)  # get file size in bytes
nchannels = spec.nchannels  # get count of channels
channels = [
    channel.replace("ViewLayer.", "") for channel in spec.channelnames
]  # get channel names

console = Console()
table = Table(
    title="EXR Image Metadata. Primary parameters", header_style="bold magenta"
)
...
table = Table(
    title=f"Значение px ({x_final}, {y_final} ({y_oiio}))", header_style="bold magenta"
)
table.add_column("Канал")
table.add_column("Значение (с флипом)")
table.add_row("R", f"{r:.5f}" if r is not None else "None")
table.add_row("G", f"{g:.5f}" if g is not None else "None")
table.add_row("B", f"{b:.5f}" if b is not None else "None")
table.add_row("A", f"{a:.5f}" if a is not None else "None")
