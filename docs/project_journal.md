...
| :------------- | :---------------- | :------------------- | :------------------------- |
| `09.08.2026`   | `10.08.2026`      | **~5 часов**         | Завершен, остались вопросы |
...
- Проблема со считыванием цвета выбранного пикселя. По моим догадкам, возможны несколько причин: 
	...
	- Разные методы чтения. Nuke берет пиксель без сглаживания (`sample` с радиусом 0,0, либо через `Pixel Analyzer`), в то время как `OpenImageIO` модет применять интерполяцию
